from scipy.interpolate import griddata, interp2d, SmoothBivariateSpline
import glob
...
def c2_warp(img, header):
	...
	return warp_tri(x0, y0, x, y, img), header
#done
def c3_warp(img, header):
	...
	w = np.arange(((image_size/gridsize)+1)**2)
	y = w / ((image_size/gridsize) + 1)
	x = w - y * ((image_size/gridsize) + 1)
	x *= gridsize
	y *= gridsize
	...
	return img[y1:y2,x1:x2], header
#done
def warp_tri(x0, y0, xi, yi, img):
	# Triangulation of output space
	tri = Delaunay(np.column_stack((x0, y0)))  # Corrected: Added missing parentheses

	# Define the surfaces (x0, y0, xi), (x0, y0, yi) with the triangulations
	xi_surf = LinearNDInterpolator(tri, xi)
	yi_surf = LinearNDInterpolator(tri, yi)

	# Define grid for output image
	output_X = np.linspace(0, img.shape[1] - 1, img.shape[1])  # Corrected: Added img.shape[1]
	output_Y = np.linspace(0, img.shape[0] - 1, img.shape[0])  # Corrected: Added img.shape[0]
	output_X, output_Y = np.meshgrid(output_X, output_Y)

	# Interpolate surfaces
	output_Xi = xi_surf(output_X, output_Y)
	output_Yi = yi_surf(output_X, output_Y)

	# Interpolate pixel values from original image
	warped_pixels = map_coordinates(img, [output_Yi.flatten(), output_Xi.flatten()], order=1, mode='reflect')

	# Reshape warped pixels to match output image shape
	warped_image = warped_pixels.reshape((img.shape[0], img.shape[1]))  # Corrected: Used img.shape to reshape
...
def reduce_statistics2(img, header, **kwargs):
	...
	wltmn = np.where(img<mnval and img!=0)
	nsatmin = len(wltmn[0])
...
def rebin(arr, *args):
	shape = (args[0], arr.shape[0] // args[0],
			 args[1], arr.shape[1] // args[1])
